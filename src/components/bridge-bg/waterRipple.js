...
class WaterRipple {
  ...
  render() {
    ...
    for (let i = 0; i < len; i += 4) {
      ...
      let next_amplitude =
        this.buffer_1[x][top] + this.buffer_1[x][bottom] + this.buffer_1[right][y] + this.buffer_1[left][y]
      next_amplitude >>= 1
  ...
  ripple(x, y) {
    ...
    const bottom = Math.min(y + this.ripple_radius, this.buffer_1[0].length - 1)
