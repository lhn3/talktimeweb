<template>
  <div :class="$style['chatting-column']">
    ...
    <div class="chatting-box" v-show="group === 0">
      <!--头部-->
      <div class="chatting-header" v-if="messageInfo.userName">
      ...
      <!--内容-->
      <chatting-body :message-list="messageInfo.messageList || []" ref="chattingBodyRef" />
      <!--底部-->
      <control-bar
        v-for="item in messageInfo.messageList || []"
    ...
    <div class="user-info" v-show="group === 1">
      <user-info-card v-if="userInfo.id" :user-info="userInfo" :send="true" :del="true" />
    </div>
  </div>
</template>
